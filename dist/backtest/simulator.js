...
exports.comparePoolBacktests = comparePoolBacktests;
// Fetch harga historis dari Birdeye API (tidak butuh key untuk public tokens)
async function fetchPriceHistory(mint, daysBack) {
...
function generateMockCandles(daysBack, volatilityPct) {
    const candles = [];
...
function simulatePosition(candles, binStep, binRange) {
    if (candles.length === 0)
        return { hoursInRange: 0, hoursOutRange: 0, estimatedIL: 0 };
...
function findOptimalBinRange(candles, binStep) {
    if (candles.length === 0 || binStep === 0)
        return 10;
    const entryPrice = candles[0].price;
...
async function runBacktest(config) {
    ...
    const verdict = netReturn > 100 ? 'excellent'
        : netReturn > 30 ? 'good'
...
function buildSummary(inRangePercent, feeApr, il, netReturn, bestRange, usedRange) {
    const lines = [];
    ...
    if (feeApr > 50)
        lines.push(`Fee APR estimasi ${feeApr.toFixed(1)}%.`);
    if (il > 5)
        lines.push(`⚠️  Estimasi IL ${il.toFixed(1)}% — hati-hati volatilitas.`);
    if (bestRange !== usedRange)
        lines.push(`Optimal bin range: ±${bestRange} (kamu pakai ±${usedRange}).`);
...
async function comparePoolBacktests(pools, daysBack = 7) {
    console.log(`\n🧪 Membandingkan ${pools.length} pool (${daysBack} hari backtest)...`);
    const results = [];
    for (const pool of pools) {
        const strategy = pool.binStep > 80 ? 'BidAsk' : pool.binStep > 20 ? 'Curve' : 'Spot';
