...
exports.formatVolatilityForAI = formatVolatilityForAI;
...
async function analyzeVolatility(tokenMint, tokenSymbol, binStep) {
...
async function swapSolToToken(connection, wallet, outputMint, solAmountLamports, slippageBps = 50) {
    try {
        // Get quote
        const quoteRes = await fetch(`${config_1.CONFIG.jupiter.apiUrl}/quote?inputMint=${SOL_MINT}&outputMint=${outputMint}` +
        ...
        const swapRes = await fetch(`${config_1.CONFIG.jupiter.apiUrl}/swap`, {
            method: 'POST',
...
async function getWalletBalances(connection, walletAddress, tokenMints) {
    try {
        const solBalance = await connection.getBalance(require('@solana/web3.js').PublicKey(walletAddress));
        return {
            sol: solBalance / 1e9,
...
function formatVolatilityForAI(data) {
    return `
