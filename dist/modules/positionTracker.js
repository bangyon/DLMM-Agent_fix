...
class PositionTracker {
    ...
    addPosition(position, currentPrice) {
    ...
    updateTrack(positionKey, currentPrice, feeEarned) {
    ...
    shouldClose(positionKey, currentPrice, pnlPercent) {
        const track = this.tracks.get(positionKey);
        if (!track)
    ...
    shouldRebalance(positionKey, isOutOfRange) {
        const track = this.tracks.get(positionKey);
        if (!track)
            return { should: false, reason: '' };
    ...
    getTrack(positionKey) {
        return this.tracks.get(positionKey);
    }
    getSummary() {
        if (this.tracks.size === 0)
            return 'Tidak ada posisi aktif';
        return Array.from(this.tracks.values()).map(t => `${t.position.poolName}: held ${t.hoursHeld.toFixed(1)}h, fee ${t.feeAccumulated.toFixed(4)} SOL, rebalance ${t.rebalanceCount}x`).join('\n');
    }
}
