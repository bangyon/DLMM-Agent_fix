...
exports.TOP_TRADER_WALLETS = exports.GMGN_TOP_WALLETS = void 0;
exports.buildTraderIntelligence = buildTraderIntelligence;
exports.getPoolConsensus = getPoolConsensus;
exports.detectExitSignals = detectExitSignals;
exports.formatIntelligenceForAI = formatIntelligenceForAI;
...
const inactiveWallets = new Map(); // wallet -> first inactive timestamp
let activeWalletList = [...exports.TOP_TRADER_WALLETS]; // mutable list, wallets pruned over time
async function fetchWalletPositions(wallet, apiKey) {
    try {
        const data = await fetchJson(`${LP_AGENT_API}/lp-positions/opening?owner=${wallet}`, apiKey);
        if (data.status !== 'success' || !data.data)
            return [];
        const gmgnStats = exports.GMGN_TOP_WALLETS[wallet];
        const label = gmgnStats
            ? `GMGN#${Object.keys(exports.GMGN_TOP_WALLETS).indexOf(wallet) + 1} WR${gmgnStats.winRate}% $${gmgnStats.pnl}`
            : 'LP Agent';
        return data.data
            .filter((p) => !p.protocol || p.protocol === 'meteora')
            .map((p) => {
            const r = p.range || [];
...
async function buildTraderIntelligence(apiKey) {
    if (!apiKey)
        return emptyIntelligence();
    console.log(`\n🧠 Fetching ${activeWalletList.length}/${exports.TOP_TRADER_WALLETS.length} top trader positions...`);
    const results = await Promise.allSettled(activeWalletList.map(w => fetchWalletPositions(w, apiKey)));
    const allPositions = [];
    let activeCount = 0;
    ...
    console.log(`   Active: ${activeCount}/${exports.TOP_TRADER_WALLETS.length} | Positions: ${allPositions.length}`);
    // Auto-prune: track inactive wallets
    // Wallet yang tidak punya posisi aktif dicatat waktu pertama kali inactive
    const now = Date.now();
    for (let i = 0; i < results.length; i++) {
        const wallet = exports.TOP_TRADER_WALLETS[i];
    ...
    if (pruned > 0)
        console.log(`   Remaining wallets: ${activeWalletList.length}`);
    // Build consensus map
    const poolMap = new Map();
    for (const pos of allPositions) {
        if (!poolMap.has(pos.pool))
            poolMap.set(pos.pool, { positions: [], traders: new Set() });
    ...
    for (const [pool, data] of poolMap) {
        ...
        const stratFreq = strategies.reduce((a, s) => { a[s] = (a[s] || 0) + 1; return a; }, {});
        ...
        const signal = traderCount >= 3 && avgWinRate >= 85 ? 'strong_entry' :
...
function getPoolConsensus(intel, pool) {
    return intel.pools.get(pool) || null;
}
function detectExitSignals(intel, pool) {
    const c = intel.pools.get(pool);
    if (!c)
        return { isExiting: false, reason: '' };
    if (c.traderExiting >= 2)
        return { isExiting: true, reason: `${c.traderExiting} top traders PnL negatif — exit signal` };
    if (c.avgPnlPercent < -5)
        return { isExiting: true, reason: `Avg PnL top traders ${c.avgPnlPercent.toFixed(1)}%` };
    return { isExiting: false, reason: '' };
}
function formatIntelligenceForAI(intel) {
    if (intel.totalPositions === 0)
        return 'Intelligence: Semua top traders sedang tidak aktif';
    const topText = intel.topPools.slice(0, 5).map((p, i) => {
        const e = p.signal === 'strong_entry' ? '🔥' : p.signal === 'entry' ? '✅' : '👀';
        const traders = p.traders.slice(0, 3).map(w => {
            const g = exports.GMGN_TOP_WALLETS[w];
...
//# sourceMappingURL=topTrader.js.map